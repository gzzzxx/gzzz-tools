...
**gzzz-tools** 是一套面向开发者的在线工具集合，整合了日常开发中最常用的 **16 个工具**，涵盖**加密、格式化、转换、时间**四大类。所有工具均在浏览器本地运行，开箱即用。
...
| 分类 | 工具 |
| :--- | :--- |
| 🔐 **加密 / 哈希** | [SM4 加密/解密](#) · [AES 加密/解密](#) · [Hash 计算](#)（MD5 / SHA-1 / SHA-256 / SHA-384 / SHA-512）· [密码强度分析](#) |
| 💻 **开发辅助** | [JSON 格式化](#) · [SQL 格式化](#)（MySQL / PostgreSQL / SQLite 等方言）· [XML 格式化](#) · [代码对比](#) · [文本统计](#) · [Crontab 表达式](#) |
| 🔄 **转换工具** | [Base64 转换](#) · [时间戳转换](#)（秒 / 毫秒）· [颜色转换](#)（HEX / RGB / HSL）· [二维码生成](#) |
| 📅 **时间工具** | [日历](#)（含农历、年历）· [计时器](#)（秒表 / 倒计时） |

> 工具数量与分类以 [`src/tools/registry.ts`](src/tools/registry.ts) 为唯一来源——新增/下线工具后，此处请同步更新。
